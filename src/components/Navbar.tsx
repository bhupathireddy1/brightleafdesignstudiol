...
const Navbar = () => {
  ...
  return (
    ...
    >
      <div className="container flex items-center justify-between px-4 md:px-6">
        <a href="#home" className="flex items-center lg:flex-none flex-1 justify-center lg:justify-start">
          <img
            ...
            className={`transition-all duration-300 w-auto object-contain ${isScrolled ? "h-16 md:h-18" : "h-16 md:h-22 lg:h-25"}`}
          />
        </a>
        ...
        <div className="hidden lg:flex items-center gap-8">
          {navLinks.map((link) => (
            <a
              key={link.name}
              href={link.href}
              className={`text-sm font-semibold transition-colors duration-300 tracking-wide ${isScrolled ? "text-foreground hover:text-primary" : "text-white hover:text-white/70"}`}
            >
              {link.name}
            </a>
          ))}
      ...
      {/* Mobile Menu */}
      <div
        className={`lg:hidden absolute top-full left-0 right-0 bg-card/98 backdrop-blur-lg transition-all duration-300 ${isMobileMenuOpen ? "opacity-100 visible" : "opacity-0 invisible"}`}
      >
